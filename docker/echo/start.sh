...
if [ -f "$PRESENCE_CHANNEL" ]; then
    # Fix 1: members.filter(function (m) { return m.socketId != member.socketId; })
    # Guard against undefined 'm' and undefined 'member' when member is already removed
    sed -i 's/members = members.filter(function (m) { return m.socketId != member.socketId; });/members = members.filter(function (m) { return m \&\& member \&\& m.socketId != member.socketId; });/' "$PRESENCE_CHANNEL"

    # Fix 2: delete member.socketId;
    # Guard against undefined 'member' when member is already removed
    sed -i 's/delete member.socketId;/if (member) { delete member.socketId; }/' "$PRESENCE_CHANNEL"
